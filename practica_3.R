...
sample(edades, size = 100, replace = T)  # el sample ordena cada vez con una distinta semilla
...
altura_sin_na <- altura[!is.na(altura)] #extraer los lementos vacios

todos <- length(altura)

length(altura_sin_na)
...
rm(list = ls()) # es para hacer una limpieza de enviroment
